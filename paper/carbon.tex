...
Consumer GPUs (4090, 5090) match each other. Datacenter GPUs (H100, A100)
match each other. Cross-tier hashes differ (documented limitation).}
\label{fig:hashes}
\end{figure}
...
on \emph{any consumer GPU} and get the same result. Consumer and datacenter
GPU clusters produce different hashes from each other (Ada/Blackwell
$\neq$ Hopper/Ampere), but are internally consistent within each tier.
